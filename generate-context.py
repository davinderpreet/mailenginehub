...
def generate_claude_md():
    """Generate concise CLAUDE.md (~25K chars). Full detail lives in REFERENCE.md."""
...
> Auto-generated by `generate-context.py` on {now}. Do NOT edit manually.
> {len(all_routes)} routes | {len(models)} models | {len(jobs)} jobs | {len(file_stats)} files ({total_lines:,} lines)
...
"""
    ...
    # Compact route groups - just route + description, no methods/line/function columns
    for group_name, group_info in ROUTE_GROUPS.items():
        doc += f"**{group_name}**: {group_info['desc']}\n"
        for path in sorted(group_info["routes"].keys()):
            desc = group_info["routes"][path]
...
if __name__ == "__main__":
    ...
    # Generate CLAUDE.md (with size guard — must stay under 30K to avoid breaking context)
    MAX_CLAUDE_MD = 30_000
