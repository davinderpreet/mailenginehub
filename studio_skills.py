...
_SYSTEM_PROMPT = (
    ...
    "You write concise, confident, technically-informed copy that converts. "
...
def _parse_json_response(text: str):
    """
    Strip markdown fences (```json ... ```) if present and parse JSON.
    ...
    if block_type:
        if block_type in _PRODUCT_BLOCKS:
            filtered = [
                ...
                if k.get("category", "").lower() in ("product", "products", "brand_copy")
            ] or knowledge
        elif block_type in _TESTIMONIAL_BLOCKS:
            filtered = [
                k for k in knowledge
                if k.get("category", "").lower() in ("testimonial", "testimonials", "review", "reviews")
            ] or knowledge
        elif block_type in _FAQ_BLOCKS:
            filtered = [
                k for k in knowledge
                if k.get("category", "").lower() in ("faq", "faqs", "support")
            ] or knowledge
...
def select_block_sequence(context: dict, provider: AIProvider) -> dict:
    ...
    user_prompt = json.dumps({
        ...
        "instructions": (
            "Select 4-8 blocks in order for this email. "
...
def compose_text(context: dict, provider: AIProvider) -> dict:
    ...
    user_prompt = json.dumps({
        ...
        "instructions": (
            ...
            "1-2 short paragraphs. Brand voice: confident, technical, Canadian. "
            "Tone: %s. "
            'Respond with JSON: {"paragraphs": ["...", "..."]}'
            ". Optionally include a \"section_header\" field if appropriate."
            % (family_key, tone)
        ),
    }, indent=2)
...
def compose_generic_block(block_type: str, context: dict, provider: AIProvider) -> dict:
    ...
    # Build the instruction with context-specific hints
    extra_hints = ""
    if block_type in _PRODUCT_BLOCKS:
        ...
        extra_hints = " Write common customer questions about driver electronics (shipping, returns, compatibility)."
    ...
    user_prompt = json.dumps({
        ...
        "instructions": (
            ...
            "Required fields: %s. "
            "Optional fields: %s. "
            "Default values for reference: %s. "
            "Brand: LDAS Electronics (Canadian electronics for drivers).%s "
    ...
    raw = provider.complete(_SYSTEM_PROMPT, user_prompt, max_tokens=1024)
    data = _parse_json_response(raw)
